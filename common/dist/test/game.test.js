"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const toolkit_1 = require("@reduxjs/toolkit");
const __1 = require("..");
const util_1 = require("./util");
describe("game initialization", () => {
    describe("game initialization", () => {
        it("should initialize (normal 5 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: "normal",
                names: util_1.SampleNames.slice(0, 5),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 5),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: ["bob", "alice", "charlie", "david", "edward"],
                    roles: ["agent", "agent", "spy", "spy", "agent"],
                    socketIDs: ["1", "0", "2", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
        it("should initialize (assassins 5 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: "assassins",
                names: util_1.SampleNames.slice(0, 5),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 5),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: ["bob", "alice", "charlie", "david", "edward"],
                    roles: ["agent", "captain", "assassin", "spy", "agent"],
                    socketIDs: ["1", "0", "2", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
        it("should initialize (custom 5 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: {
                    assassin: true,
                    captain: true,
                    deputy: true,
                    imposter: true,
                    intern: true,
                    mole: true,
                },
                names: util_1.SampleNames.slice(0, 5),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 5),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: ["bob", "alice", "charlie", "david", "edward"],
                    roles: ["deputy", "agent", "imposter", "assassin", "captain"],
                    socketIDs: ["1", "0", "2", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
        it("should initialize (normal 8 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: "normal",
                names: util_1.SampleNames.slice(0, 8),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 8),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: [
                        "charlie",
                        "bob",
                        "harry",
                        "george",
                        "alice",
                        "fred",
                        "david",
                        "edward",
                    ],
                    roles: [
                        "spy",
                        "agent",
                        "agent",
                        "agent",
                        "spy",
                        "agent",
                        "agent",
                        "spy",
                    ],
                    socketIDs: ["2", "1", "7", "6", "0", "5", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
        it("should initialize (assassins 8 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: "normal",
                names: util_1.SampleNames.slice(0, 8),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 8),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: [
                        "charlie",
                        "bob",
                        "harry",
                        "george",
                        "alice",
                        "fred",
                        "david",
                        "edward",
                    ],
                    roles: [
                        "spy",
                        "agent",
                        "agent",
                        "agent",
                        "spy",
                        "agent",
                        "agent",
                        "spy",
                    ],
                    socketIDs: ["2", "1", "7", "6", "0", "5", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
        it("should initialize (custom 8 players)", () => {
            const store = (0, toolkit_1.configureStore)({
                reducer: __1.GameReducer,
            });
            store.dispatch(__1.GameAction.initialize({
                gamemode: {
                    assassin: true,
                    captain: true,
                    deputy: true,
                    imposter: true,
                    intern: true,
                    mole: true,
                },
                names: util_1.SampleNames.slice(0, 8),
                seed: 0,
                socketIDs: util_1.SampleIDs.slice(0, 8),
            }));
            expect(store.getState()).toEqual({
                assassinChoice: null,
                chat: [],
                game: {
                    mission: 0,
                    phase: "role-reveal",
                    phaseCountdown: 10,
                },
                mission: null,
                missionHistory: [],
                player: {
                    names: [
                        "charlie",
                        "bob",
                        "harry",
                        "george",
                        "alice",
                        "fred",
                        "david",
                        "edward",
                    ],
                    roles: [
                        "intern",
                        "agent",
                        "deputy",
                        "agent",
                        "imposter",
                        "captain",
                        "agent",
                        "assassin",
                    ],
                    socketIDs: ["2", "1", "7", "6", "0", "5", "3", "4"],
                },
                statusMessage: "Welcome to The Resistance",
                team: null,
                teamHistory: [],
                winner: null,
            });
        });
    });
    it("should be able to tick", () => {
        const store = (0, util_1.getInitializedGame)("normal");
        store.dispatch(__1.GameAction.tick());
        expect(store.getState().game.phaseCountdown).toEqual(9);
    });
    it("should have the first person as the leader", () => {
        const store = (0, util_1.getInitializedGame)("normal");
        for (let i = 0; i < 11; i++) {
            store.dispatch(__1.GameAction.tick());
        }
        expect(store.getState().team.leader).toEqual(0);
    });
});
describe("basic game cases", () => {
    it("should fail missions", () => {
        const store = (0, util_1.getInitializedGame)("normal");
        for (let i = 0; i < 3; i++) {
            (0, util_1.tickUntil)(store, "team-building");
            (0, util_1.addTeamMembers)(store, true);
            store.dispatch(__1.GameAction.finishTeamBuilding());
            (0, util_1.tickUntil)(store, "voting");
            (0, util_1.voteAll)(store, "accept");
            (0, util_1.tickUntil)(store, "mission");
            (0, util_1.missionAll)(store, "fail");
        }
        (0, util_1.tickUntil)(store, "finished");
        expect(store.getState().missionHistory.length).toEqual(3);
        expect(store.getState().winner).toEqual("spy");
    });
    it("should succeed missions", () => {
        const store = (0, util_1.getInitializedGame)("normal");
        for (let i = 0; i < 3; i++) {
            (0, util_1.tickUntil)(store, "team-building");
            (0, util_1.addTeamMembers)(store, false);
            store.dispatch(__1.GameAction.finishTeamBuilding());
            (0, util_1.tickUntil)(store, "voting");
            (0, util_1.voteAll)(store, "accept");
            (0, util_1.tickUntil)(store, "mission");
            (0, util_1.missionAll)(store, "success");
        }
        (0, util_1.tickUntil)(store, "finished");
        expect(store.getState().missionHistory.length).toEqual(3);
        expect(store.getState().winner).toEqual("agent");
    });
});
describe("advanced game cases", () => {
    it("should reject vote on ties", () => {
        const store = (0, util_1.getInitializedGame)("normal", 6);
        (0, util_1.tickUntil)(store, "team-building");
        (0, util_1.addTeamMembers)(store);
        store.dispatch(__1.GameAction.finishTeamBuilding());
    });
    it("should fail on hammer", () => {
        const store = (0, util_1.getInitializedGame)("normal");
        for (let i = 0; i < 5; i++) {
            (0, util_1.tickUntil)(store, "team-building");
            (0, util_1.addTeamMembers)(store);
            store.dispatch(__1.GameAction.finishTeamBuilding());
        }
        (0, util_1.tickUntil)(store, "finished");
        expect(store.getState().winner).toEqual("spy");
    });
});
//# sourceMappingURL=game.test.js.map